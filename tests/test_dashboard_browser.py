...
# this test module's own private database copy -- never to the shared
# scripts/build_coherent_demo.py fixture every other test module also
# rebuilds and asserts exact known values against. This exists so the
# Player vs Player browser tests below can prove genuine cross-scope
# selector filtering deterministically, instead of depending on whatever
# opponent diversity happens to exist in a live account's database.
...
def _add_second_opposing_team_scope(db_path: str) -> None:
    """Ingest one real, finalized, scored match (and its real head-to-head
    row) between build_coherent_demo's existing team and a brand-new second
    opposing team, using the same real database.ingest.* functions
    build_coherent_demo.build() itself calls -- so the resulting second
    scope is exactly as authoritative as the first, just smaller."""
    ...
    with Session(engine) as db:
        ...
        ingest_match(
            db, match_id=SECOND_SCOPE_MATCH_ID,
            home_team_id=build_coherent_demo.OUR_TEAM_ID, away_team_id=SECOND_OPP_TEAM_ID,
            home_team_name=build_coherent_demo.OUR_TEAM_NAME, away_team_name=SECOND_OPP_TEAM_NAME,
            match_date="2026-08-15", status="COMPLETED",
            home_score=3, away_score=1, week=8,
            is_bye=False, is_scored=True, is_finalized=True,
            format=build_coherent_demo.FORMAT_NAME, session_name=build_coherent_demo.SESSION_NAME,
        )
        ingest_match_scores(db, SECOND_SCOPE_MATCH_ID, [
            {
                "player_id": our_ext, "player_name": our_name, "team_id": build_coherent_demo.OUR_TEAM_ID,
                "team_name": build_coherent_demo.OUR_TEAM_NAME, "skill_level": our_skill,
                "result": "W", "points_earned": 3,
            },
            {
                "player_id": opp_ext, "player_name": opp_name, "team_id": SECOND_OPP_TEAM_ID,
                "team_name": SECOND_OPP_TEAM_NAME, "skill_level": opp_skill,
                "result": "L", "points_earned": 1,
            },
        ])
        ingest_head_to_head(db, SECOND_SCOPE_MATCH_ID, [
            {
        ...
        ingest_player_team_history(db, player, [{
            "team_id": SECOND_OPP_TEAM_ID, "team_name": SECOND_OPP_TEAM_NAME,
            "division_id": build_coherent_demo.DIVISION_ID, "session_name": build_coherent_demo.SESSION_NAME,
            "is_current": True, "skill_level": opp_skill,
            "matches_won": 0, "matches_played": 1,
        }])
        build_matchups(db)
...
class TestPlayerVsPlayerSelectors:
    ...
    def test_opposing_team_then_player_selector_matches_exact_scope(self, page):
        ...
        # Find the one player the fixture deliberately gives two distinct
        # real opposing-team scopes (see _add_second_opposing_team_scope
        # above) -- proving cross-scope filtering requires a player who
        # actually HAS two scopes to filter between, not just one.
        ...
        for candidate_id, choices in opponent_index.items():
            if len({scope_key_of(c) for c in choices}) >= 2:
