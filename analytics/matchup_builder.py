...
def build_matchups(db: Session) -> list[dict]:
    ...
    for (player_id, opponent_id, format_, session_name), h2h_rows in by_pair.items():
        ...
        # over the last 5 readings within this format/session-scoped history.
        # See analytics.skill_level_trends.normalized_volatility for why the
        # rate is divided by real transition opportunities rather than using
        # a capped raw change count.
