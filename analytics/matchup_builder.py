...
from analytics.skill_level_trends import normalized_volatility, skill_level_trend
...
def build_matchups(db: Session) -> list[dict]:
    ...
    for (player_id, opponent_id, format_, session_name), h2h_rows in by_pair.items():
        ...
        # P2 volatility normalization: a RATE (changes / valid transitions)
        # over the last 5 readings within this (format, session)-scoped
        # history -- see analytics.skill_level_trends.normalized_volatility
        # for why it divides by transitions rather than counting changes,
        # and why it stays separate from skill_level_volatility.
        volatility = normalized_volatility(own_history)
        ...
        rows.append(
            {
                ...
                # P2 SL delta context -- unweighted, includes losses -- see
                # analytics.matchups.average_own_skill_level/
                # average_skill_level_delta's docstrings for how these
                # differ from the win-scoped, score-weighted
                # opponent_skill_modifier already folded into matchup_score.
                "avg_own_skill_level": average_own_skill_level(h2h_rows),
