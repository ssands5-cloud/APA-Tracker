    ...
    rows = [
        {
            ...
            "avg_own_skill_level": r.avg_own_skill_level,
            "sl_delta": r.sl_delta,
            "trend": r.trend,
            ...
            rows.append(
                {
                    ...
                    "avg_opponent_skill_level": None, "avg_own_skill_level": None, "sl_delta": None,
                    "trend": "no data", "volatility": 0,
...
def player_trends(db: Session) -> list[PlayerTrend]:
    """Every player trend, strongest upward slope first.

    Ordered so the sheet and the demo tab open on the players moving most,
    without either having to sort. NULL slopes (too little history for a
    trend) sort last rather than reading as zero.
    """
    return (
